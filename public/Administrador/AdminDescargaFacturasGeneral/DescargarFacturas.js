...
function renderTabla() {
  ...
  if (!FACTURAS_VIEW.length) {
    tbody.innerHTML = `
      <tr>
        <td colspan="15" style="text-align:center; padding:16px; color: var(--text-secondary);">
...
function applyFilters() {
  ...
  // NUEVO: valor del filtro de ID Ticket
  ...
  FACTURAS_VIEW = FACTURAS_ALL.filter(f => {
    ...
    // NIT Emisor (contiene)
    if (nitQ) {
      const nit = String(f.nitEmisor || "").toLowerCase();
      if (!nit.includes(nitQ)) return false;
    }

    // NUEVO: ID Ticket (contiene)
...
function descargarCSV() {
  ...
  const headers = [
    "ID Ticket","ID Administrador","ID Factura","ID Usuario",
