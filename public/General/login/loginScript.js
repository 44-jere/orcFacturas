...
function updateBlockDisplay() {
    ...
    if (appState.isBlocked) {
        ...
        if (blockTimeLeft) blockTimeLeft.textContent = formatTime(appState.blockTimeLeft);
    } else {
...
function updateLoadingState() {
    ...
    // Actualizar botón
    if (loginBtn) {
        loginBtn.disabled = appState.isLoading || appState.isBlocked || !appState.formData.usuario || !appState.formData.contraseña;
    }
    ...
    try {
        ...
        if (result.success) {
            ...
            if (appState.loginAttempts >= MAX_LOGIN_ATTEMPTS) {
                ...
                appState.errors = { 
                    general: `Demasiados intentos fallidos. Bloqueado por ${BLOCK_TIME/60} minutos.` 
                };
                startBlockTimer();
            } else {
                appState.errors = { 
...
async function handleForgotPassword() {
    ...
    try {
        const response = await fetch(`${API_CONFIG.baseURL}${API_CONFIG.forgotPasswordEndpoint}`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
        ...
        if (response.ok) {
            ...
            showNotification(data.message || "Error al enviar el enlace de recuperación", 'error');
        }
    } catch (error) {
